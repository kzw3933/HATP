...
from utils import create_dir_if_not_exists, get_root_path, natural_keys, LLVMParser, Debuger
...
def add_depth_info(g):
    """add the depth info to each node in the graph
    """
    ...
    for node, ndata in g.nodes(data = True):
        block_id = ndata['block']
        try:
            depth = LLVM_PARSER.get_block_depth(block_id)
        except:
            depth = 0
        
        ndata['depth'] = depth
        
        new_nodes.append((node, ndata))
        
    add_to_graph(g_new, nodes = new_nodes, edges = new_edges)
        
    return g_new

def augment_graph_hierarchy(name, for_blocks_info, src_path, dst_path, csv_dict=None, node_type = 'block', add_depth = False):
    if node_type == 'block':
        ...
        if csv_dict: csv_dict[name] = current_g_value
        if add_depth: add_depth_info(g_new)
        nx.write_gexf(g_new, new_gexf_file)
    ...
    if mode == 'initial':
        ...
        for BENCHMARK in target:
            ...
            for kernel in ALL_KERNEL[BENCHMARK]:
                ...
                LLVM_PARSER.set_src(join(llvm_base_folder, kernel, f'{kernel}.ll'))
                augment_graph_hierarchy(kernel, for_blocks_info, src_path = auxiliary_node_gexf_folder, dst_path = dest_path, csv_dict=csv_dict, add_depth=True)
                print()
        write_csv_file(csv_dict, csv_header, f'{type_graph}/{mode}.csv')
    else:
        raise NotImplementedError()
