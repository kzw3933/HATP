...
def read_fcontent(path):
    with open(join(get_root_path(), path), "r") as f:
        return f.read()
    
class LLVMParser:    
    def __init__(self) -> None:
        self.module = OrderedDict()
        self.blockid2depth =  OrderedDict()       
        
    def set_src(self, src_code_path):
        content = read_fcontent(join(get_root_path(), src_code_path))
        self.reset()
        self.parse_module(content)
        self.parse_block_depth()
                
    def get_block_id(self, func_name, block_label):
        return self.module[func_name]['block2id'][block_label]  
    
    def get_block_depth(self, block_id):
        return self.blockid2depth[block_id]
        
    def reset(self):
        self.module.clear()
        self.blockid2depth.clear()
    
    def parse_module(self, src_code: str):
        _lines = src_code.splitlines()
        func_scope = False
        cur_func_name = None
        cur_block_name = None
        cur_block_idx = -1
        ...
        for line in _lines:
            ...
            if func_scope:
                if line.startswith('}'):
                    ...
                    cur_block_idx += 1
                    cur_block_name = line
                    self.module[cur_func_name]['blocks'][cur_block_name] = []
                    block_label = cur_block_name.split(':')[0].strip()
                    self.module[cur_func_name]['block2id'][block_label] = cur_block_idx
                else:
                    if line:
                        self.module[cur_func_name]['blocks'][cur_block_name].append(line)
                    
            else:
                if line.startswith('define'):
                    func_scope = True
                    cur_func_name = line.split('@')[1].split('(')[0]
                    self.module[cur_func_name] = OrderedDict()
                    self.module[cur_func_name]['blocks'] = OrderedDict()
                    self.module[cur_func_name]['block2id'] = OrderedDict()
                            
        
    def parse_block_depth(self):
        for func_name, func in self.module.items():
            for_depth = 0
            for block_name, block in func['blocks'].items():
                block_label = block_name.split(':')[0].strip()
                if 'for.cond' in block_label:
                    for_depth += 1
                    self.blockid2depth[self.get_block_id(func_name, block_label)] = for_depth
                elif 'for.end' in block_label:
                    self.blockid2depth[self.get_block_id(func_name, block_label)] = for_depth
                    for_depth -= 1
                else:
                    self.blockid2depth[self.get_block_id(func_name, block_label)] = for_depth
...
class Debuger:
    def __init__(self) -> None:
        """start debug by vscode with shell-style run
