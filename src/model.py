...
class Net(nn.Module):
    ...
    def forward(self, data):
        x, edge_index, edge_attr, batch, pragmas, edge5to4, edge4t03, edge3to2, edge2to1, edge1to0 = \
            data.x, data.edge_index, data.edge_attr, data.batch , data.pragmas, data.edge5to4, data.edge4t03, data.edge3to2, data.edge2to1, data.edge1to0
        ...
        if self.task == 'regression':
            out = self.loop_gcn(out, edge5to4) 
            out = self.loop_gcn(out, edge4t03)
            out = self.loop_gcn(out, edge3to2)
            out = self.loop_gcn(out, edge2to1)
            out = self.loop_gcn(out, edge1to0)
