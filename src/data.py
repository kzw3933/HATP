...
if FLAGS.dataset == 'harp':
    GEXF_FOLDER = join(get_root_path(), 'dse_database', 'kaggle/train_data/data', 'extend_graphs', '**')
else:
...
class MyData(Data):
    def __inc__(self, key, value, *args, **kwargs):
        # 针对自定义的 edge_index 进行处理
        if 'edge' in key:
            return self.x.size(0)  # 节点数作为增量
        else:
            return super().__inc__(key, value, *args, **kwargs)
    def __cat_dim__(self, key, value, *args, **kwargs):
        # 针对自定义的 edge_index，按列进行拼接
        if 'edge' in key:
            return -1  # 按列拼接
        else:
...
def get_data_list():
    ...
    for gexf_file in tqdm(GEXF_FILES[0:]): 
        ...
        design_path = osp.join(get_root_path(), f"dse_database/kaggle/train_data/data/designs/{FLAGS.v_db}/"+kernel_name+".json")
        ...
        edge5to4, edge4to3, edge3to2, edge2to1, edge1to0 = split_child_graph_by_depth(g)

        cnt = 0
        for key in sorted(keys):
            
            design = designs[key]
            obj = Design(kernel_name, FLAGS.v_db, design, is_inference=False)
...
def split_child_graph_by_depth(g):
    depth0_nodes, depth1_nodes, depth2_nodes, depth3_nodes, depth4_nodes, depth5_nodes = [], [], [], [], [], []
    edge5to4, edge4to3, edge3to2, edge2to1, edge1to0 = [], [], [], [], []
    
    for nid, (node, ndata) in enumerate(g.nodes(data=True)):
        if ndata['depth'] == 0:
            depth0_nodes.append((node, ndata))
        elif ndata['depth'] == 1:
            depth1_nodes.append((node, ndata))
        elif ndata['depth'] == 2:
            depth2_nodes.append((node, ndata))
        elif ndata['depth'] == 3:
            depth3_nodes.append((node, ndata))
        elif ndata['depth'] == 4:
            depth4_nodes.append((node, ndata))
        elif ndata['depth'] == 5:
            depth5_nodes.append((node, ndata))
            
    def build_conn(src_nodes, tgt_nodes, edges):  
        ...
        for (src, src_data) in src_nodes:
            for (tgt, tgt_data) in src_nodes:
                if 'text' in tgt_data and 'pseudo' in tgt_data['text'] and src_data['block'] == tgt_data['block'] and src != tgt:
                    edges.append([int(src), int(tgt)])
                    
    build_conn(depth5_nodes, depth4_nodes, edge5to4)
    build_conn(depth4_nodes, depth3_nodes, edge4to3)
    build_conn(depth3_nodes, depth2_nodes, edge3to2)
    build_conn(depth2_nodes, depth1_nodes, edge2to1)
    build_conn(depth1_nodes, depth0_nodes, edge1to0)
    
    return torch.LongTensor(np.array(edge5to4)), torch.LongTensor(np.array(edge4to3)), torch.LongTensor(np.array(edge3to2)), torch.LongTensor(np.array(edge2to1)), torch.LongTensor(np.array(edge1to0))
